...
import { CreateStoryBody, ListStoriesQuery } from './stories/stories-schemas.js';
...
export const StoriesController = {
  ...
  async createStory(request: FastifyRequest<{ Body: CreateStoryBody }>, reply: FastifyReply) {
    try {
      ...
      const story = await StoriesService.createStory({ userId, mediaType, mediaUrl, thumbnailUrl, caption });
  ...
  async getStory(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
  ...
  async getStoriesByUser(request: FastifyRequest<{ Params: { userId: string } }>, reply: FastifyReply) {
    try {
