...
} from './missions/missions-schemas.js';

export class MissionsController {
  /**
   * POST /missions - Create a new mission (admin only)
   */
  static async createMission(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while creating the mission' },
      });
    }
  }

  /**
   * GET /missions - List missions with pagination
   */
  static async listMissions(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const result = await MissionsService.listMissions({
        limit: query.limit, offset: query.offset, type: query.type, isActive: query.isActive,
      });
      return reply.code(200).send({ success: true, data: result.missions, pagination: result.pagination });
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.code(400).send({
          success: false,
      ...
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while fetching missions' },
      });
    }
  }

  /**
   * GET /missions/today - Get today's active mission
   */
    ...
    try {
      ...
      if (!mission) {
        return reply.code(404).send({
          success: false,
          error: { code: 'NO_ACTIVE_MISSION', message: 'No active mission for today' },
        });
      }
      return reply.code(200).send({ success: true, data: mission });
    } catch (error) {
      request.log.error({ error }, 'Error getting today\'s mission');
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while fetching today\'s mission' },
      });
    }
  }
  ...
  static async getMissionById(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      ...
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while fetching the mission' },
      });
    }
  }

  /**
   * PATCH /missions/:id - Update a mission (admin only)
   */
  static async updateMission(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      ...
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while updating the mission' },
      });
    }
  }

  /**
   * DELETE /missions/:id - Delete a mission (admin only)
   */
  static async deleteMission(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      ...
      return reply.code(200).send({ success: true, message: 'Mission deleted successfully' });
    } catch (error) {
      request.log.error({ error }, 'Error deleting mission');
      return reply.code(500).send({
        success: false,
        error: { code: 'INTERNAL_SERVER_ERROR', message: 'An error occurred while deleting the mission' },
      });
    }
  }
}
