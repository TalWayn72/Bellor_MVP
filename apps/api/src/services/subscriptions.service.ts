...
export const SubscriptionsService = {
  ...
  async reactivateSubscription(userId: string) {
    ...
    await stripeBreaker.execute(() =>
      stripeClient.subscriptions.update(subscription.stripeSubscriptionId!, { cancel_at_period_end: false }),
    );
    await prisma.subscription.update({
      where: { id: subscription.id },
  ...
  async createPlan(data: {
    ...
    const mp = await stripeBreaker.execute(() =>
      s.prices.create({ product: product.id, unit_amount: Math.round(data.price * 100), currency: 'usd', recurring: { interval: 'month' } }),
    );
    let yp = null;
    if (data.priceYearly) {
      yp = await stripeBreaker.execute(() =>
        s.prices.create({ product: product.id, unit_amount: Math.round(data.priceYearly! * 100), currency: 'usd', recurring: { interval: 'year' } }),
      );
    }
