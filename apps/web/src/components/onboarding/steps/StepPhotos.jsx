          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="relative aspect-square">
                ...
                {formData.profile_images[i] && (
                  <>
                    <button onClick={(e) => setMainImage(e, formData.profile_images[i])} className={`absolute top-2 left-2 w-7 h-7 rounded-full flex items-center justify-center shadow-lg z-10 ${formData.main_profile_image_url === formData.profile_images[i] ? 'bg-info/100 text-white' : 'bg-white text-muted-foreground hover:bg-muted'}`} title="Set as main profile photo">
